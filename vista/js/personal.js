...
new class Personal {
    ...
    constructor() {
        ...
        this.columnas = [ // este array de objetos define las columnas de la tabla
            ...
            { title: 'ID Personal', field: 'id_persona', editor: 'input', width: 100, align: 'center' },
            { title: 'Nombre', field: 'nombre', editor: 'input', width: 270 },
            { title: 'Dirección', field: 'direccion', editor: 'input' },
            { title: 'Teléfonos', field: 'telefono', editor: 'input', align: 'center' },
            { title: 'Perfil', field: 'perfil', editor: 'input', align: 'center' },
        ...
        M.FormSelect.init($('#personal-selectperfil'));
    ...
    insertarRegistro() {
        ...
        if($('#personal-selectperfil').value==""){
            M.toast({ html: 'Por Favor seleccione un Perfil',color:'green' });
            return;
        }
        if($('#personal-txtcontrasena').value.trim()=='' || $('#personal-txtcontrasena').value!=$('#personal-txtcontrasena-confirm').value){
            M.toast({ html: 'Contraseña no coincide o campo contraseña vacio, Por favor intente nuevamente',color:'green' });
            return;
        }
        let nuevoPersonal = {
            id_persona: $('#personal-txtid').value,
            nombre: $('#personal-txtnombre').value,
            direccion: $('#personal-txtdireccion').value,
            telefono: $('#personal-txttelefono').value,
            perfil: $('#personal-selectperfil').value,
            contrasena: $('#personal-txtcontrasena').value,
        };
        ...
        util.fetchData('./controlador/fachada.php', {
            ...
            if (data.ok) {
                ...
                $('#personal-txtid').value = '';
                $('#personal-txtnombre').value = '';
                $('#personal-txtdireccion').value = '';
                $('#personal-txttelefono').value = '';
                $('#personal-selectperfil').value = '';
                $('#personal-txtcontrasena').value = '';
                $('#personal-txtcontrasena-confirm').value = '';
            } else {
                throw new Error(data.mensaje);
            }
        }).catch(error => {
            util.mensaje(error, 'No se pudo insertar el personal');
        });
    }
    ...
    editarRegistro() {
        ...
        $('#personal-txtid').value = filaActual.id_persona;
        ...
        $('#personal-selectperfil').value = filaActual.perfil;
        $('#personal-txtcontrasena').value = filaActual.contrasena;
        $('#personal-txtcontrasena-confirm').value = filaActual.contrasena;
    ...
    actualizarRegistro() {
        ...
        let nuevosDatosPersonal = {
            ...
            id_persona: $('#personal-txtid').value, // el posible nuevo ID
            nombre: $('#personal-txtnombre').value,
            direccion: $('#personal-txtdireccion').value,
            telefono: $('#personal-txttelefono').value,
            perfil: $('#personal-selectperfil').value,
     ...
     */
    eliminarRegistro() {
        let idFila = this.filaActual.getData().id_persona;

        MaterialDialog.dialog(
            "Va a eliminar un empleado, por favor confirme la accion",{
                title:"Cuidado",
                buttons:{
                    close:{
                        className:"red",
                    ...
                    confirm:{
                        ...
                        callback : () =>{
                            ...
                            // se envía el ID del personal al back-end para el eliminado y se actualiza la tabla
                            util.fetchData('./controlador/fachada.php', {
                                'method': 'POST',
                                'body': {
                                    clase: 'Personal',
                                ...
                                util.mensaje(error, `No se pudo eliminar el personal con ID ${idFila}`);
                            });
