...
// dicho objeto define la gestión de proveedores, utilizando el componente 'Tabulator' (http://tabulator.info/)

new class Proveedor {

    constructor() {

        this.contenedor = '#tabla-proveedores'; // el div que contendrá la tabla de datos de proveedores
        ...
        this.columnas = [ // este array de objetos define las columnas de la tabla
            ...
            { title: 'ID Proveedor', field: 'id_proveedor', editor: 'input', width: 150, align: 'center' },
            { title: 'Nombre', field: 'nombre', editor: 'input', width: 270 },
            { title: 'Telefono', field: 'telefono', editor: 'input' },
            { title: 'Correo Electronico', field: 'correo', editor: 'input', align: 'center' },
        ];
    ...
    /**
     * Se asignan los eventos a los botones principales para la gestión de proveedores
     */
    gestionarEventos() {
        $('#proveedor-btnagregar').addEventListener('click', event => {
            this.operacion = 'insertar';
            // despliega el formulario para editar proveedores. Ir a la definición del boton 
            // 'proveedor-btnagregar' en proveedores.html para ver cómo se dispara este evento
        });
    ...
    /**
     * Envía un nuevo registro al back-end para ser insertado en la tabla proveedores
     */
    insertarRegistro() {
        // se creas un objeto con los datos del formulario
        let nuevoProveedor = {
            id_proveedor: $('#proveedor-txtid').value,
        ...
        // se envían los datos del nuevo proveedor al back-end y se nuestra la nueva fila en la tabla
        util.fetchData('./controlador/fachada.php', {
            ...
            util.mensaje(error, 'No se pudo insertar el proveedor');
        });
    }
    ...
    editarRegistro() {
        ...
        $('#proveedor-txtid').value = filaActual.id_proveedor;
    ...
    actualizarRegistro() {
        ...
        let nuevosDatosProveedor = {
            ...
            id_proveedor: $('#proveedor-txtid').value, // el posible nuevo ID
            nombre: $('#proveedor-txtnombre').value,
            telefono: $('#proveedor-txttelefono').value,
            correo: $('#proveedor-txtcorreo').value,
     ...
     */
    eliminarRegistro() {
        let idFila = this.filaActual.getData().id_proveedor;

        MaterialDialog.dialog(
            "Va a eliminar un proveedor, por favor confirme la accion",{
                title:"Cuidado",
                buttons:{
                    close:{
                        className:"Red",
                    ...
                    confirm:{
                        ...
                        text:"Confirmar",
                        callback: () =>{
                            // se envía el ID del proveedor al back-end para el eliminado y se actualiza la tabla
                            util.fetchData('./controlador/fachada.php', {
                                'method': 'POST',
                                'body': {
                                    clase: 'Proveedor',
                                ...
                                util.mensaje(error, `No se pudo eliminar el proveedor con ID ${idFila}`);
                            });
