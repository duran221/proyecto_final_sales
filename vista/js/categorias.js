...
// dicho objeto define la gestión de categorias, utilizando el componente 'Tabulator' (http://tabulator.info/)
...
new class CategoriaProducto {
    ...
    constructor() {
        ...
        this.columnas = [ // este array de objetos define las columnas de la tabla
            ...
            { title: 'ID Categoria Producto', field: 'id_categoria_producto',visible:false},
            { title: 'Nombre', field: 'nombre', editor: 'input', align:'center' },
        ...
        this.frmEdicionCategoriaProducto = M.Modal.init($('#categorias-frmedicion'), {
    ...
    /**
     * Se asignan los eventos a los botones principales para la gestión de categorias
     */
    gestionarEventos() {
        $('#categorias-btnagregar').addEventListener('click', event => {
            this.operacion = 'insertar';
            // despliega el formulario para editar categorias. Ir a la definición del boton 
            // 'categorias-btnagregar' en categorias.html para ver cómo se dispara este evento
        });

        $('#categorias-btnaceptar').addEventListener('click', event => {
            ...
            this.frmEdicionCategoriaProducto.close();
        });

        $('#categorias-btncancelar').addEventListener('click', event => {
            this.frmEdicionCategoriaProducto.close();
        });
    }

    /**
     * Envía un nuevo registro al back-end para ser insertado en la tabla categorias
     */
    insertarRegistro() {
        // se creas un objeto con los datos del formulario
        let nuevoCategoriaProducto = {
            nombre: $('#categorias-txtnombre').value
        };

        // se envían los datos del nuevo categorias al back-end y se nuestra la nueva fila en la tabla
        util.fetchData('./controlador/fachada.php', {
            'method': 'POST',
            'body': {
                clase: 'CategoriaProducto',
                accion: 'insertar',
                data: nuevoCategoriaProducto
            }
        }).then(data => {
            if (data.ok) {
                util.mensaje('', '<i class="material-icons">done</i>', 'teal darken');
                nuevoCategoriaProducto['id_categoria_producto'] = data.id_categoria_producto;
                this.tabla.addData([nuevoCategoriaProducto]);
                $('#categorias-txtid').value = '';
                $('#categorias-txtnombre').value = '';
            } else {
                throw new Error(data.mensaje);
            }
        }).catch(error => {
            util.mensaje(error, 'No se pudo insertar la categoria');
        });
    }
     ...
     */
    editarRegistro() {
        this.frmEdicionCategoriaProducto.open();
        // se muestran en el formulario los datos de la fila a editar
        let filaActual = this.filaActual.getData();
        $('#categorias-txtid').value = filaActual.id_categoria_producto;
        $('#categorias-txtnombre').value = filaActual.nombre;
        M.updateTextFields();
    }
    ...
    actualizarRegistro() {
        ...
        let idCategoriaProductoActual = this.filaActual.getData().id_categoria_producto;
        let nuevosDatosCategoriaProducto = {
            id_actual: idCategoriaProductoActual,
            id_categoria_producto: $('#categorias-txtid').value, // el posible nuevo ID
            nombre: $('#categorias-txtnombre').value,
        };
        ...
        util.fetchData('./controlador/fachada.php', {
            ...
            'body': {
                clase: 'CategoriaProducto',
                accion: 'actualizar',
                data: nuevosDatosCategoriaProducto
            }
        }).then(data => {
            if (data.ok) {
                util.mensaje('', '<i class="material-icons">done</i>', 'teal darken');
                delete nuevosDatosCategoriaProducto.id_actual; // elimina esta propiedad del objeto, ya no se requiere
                this.tabla.updateRow(idCategoriaProductoActual, nuevosDatosCategoriaProducto);
            } else {
                throw new Error(data.mensaje);
            }
        }).catch(error => {
    ...
    eliminarRegistro() {
        ...
        MaterialDialog.dialog(
            "¡Cuidado! Va a Eliminar una Categoria, por favor confirme una accion",{
                title:"Cuidado",
                buttons:{
                    close:{
                        className:"red",
                    ...
                    confirm:{
                        ...
                        callback: () =>{
                             ...
                             // se envía el ID del categorias al back-end para el eliminado y se actualiza la tabla
                            util.fetchData('./controlador/fachada.php', {
                                'method': 'POST',
                                'body': {
                                    clase: 'CategoriaProducto',
                                    accion: 'eliminar',
                                    id_categoria_producto: idFila
                                }
                            }).then(data => {
                                if (data.ok) {
                                ...
                                util.mensaje(error, `No se pudo eliminar la categoria con ID ${idFila}`);
                            });
